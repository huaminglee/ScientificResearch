...
namespace CCFlow.WF.Comm.UC
{
    ...
    public partial class UCSys : BP.Web.UC.UCBase
    {
        ...
        public void DataPanelDtl(Entities ens, string ctrlId)
        {
            ...
            // this.AddTable("");
            this.AddTR();
            this.AddTDGroupTitle("text-align:center", "序");
            foreach (Attr attrT in selectedAttrs)
            {
                if (attrT.UIVisible == false)
                    continue;
            ...
            foreach (Entity en in ens)
            {
                ...
                {
                    string str = "";
                    // string str = "<a href=\"javascript:WinOpen('../Comm/RefFunc/UIEn.aspx?EnsName=" + ens.ToString() + "&PK=" + en.GetValByKey(pk) + url + "', 'cd')\" >打开</a>";
                    //<a href=\"javascript:WinOpen('../Comm/RefFunc/UIEn.aspx?EnsName=" + ens.ToString() + "&PK=" + en.GetValByKey(pk) + url + "','cd','400','600')\" >打开</A>
