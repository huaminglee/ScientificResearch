         ...
         //   BP.Sys.SystemConfig.DoClearCash();
            ...
            switch (this.DoType)
            {
                    ...
                    switch (this.EnName)
                    {
                        case "BP.WF.Flow":
                            ...
                            if (fl.FlowAppType == FlowAppType.DocFlow)
                                this.Response.Redirect("../../Comm/RefFunc/UIEn.aspx?EnsName=BP.WF.Template.Ext.FlowDocs&No=" + this.PK, true);
                            else
                                this.Response.Redirect("../../Comm/RefFunc/UIEn.aspx?EnsName=BP.WF.Template.Ext.FlowSheets&No=" + this.PK, true);
                            return;
                        case "BP.WF.Node":
                            Node nd = new Node(this.PK);
                            this.Response.Redirect("../../Comm/RefFunc/UIEn.aspx?EnsName=BP.WF.Template.Ext.NodeSheets&PK=" + this.PK, true);
                            return;
                        case "BP.WF.FlowSort":
                ...
                case "FlowFrms": //"流程表单"
                    ...
                    break;
                case "StaDef": // 节点岗位.
                    this.Response.Redirect("../../Comm/RefFunc/Dot2Dot.aspx?EnName=BP.WF.Template.Ext.NodeSheet&AttrKey=BP.WF.Template.NodeStations&PK=" + this.PK + "&NodeID=" + this.PK + "&RunModel=0&FLRole=0&FJOpen=0&r=" + this.PK, true);
                    break;
                case "WFRpt": // 报表设计.r
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(FK_MapData))
                {
                    ...
                    { // 流程表单
                        ...
                        path = BP.Sys.SystemConfig.PathOfDataUser + @"FlowDesc\FlowForm\" + xmlName + "\\";// 流程表单
                    }
                }
