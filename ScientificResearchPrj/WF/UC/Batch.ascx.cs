...
namespace CCFlow.WF.UC
{
    public partial class Batch : BP.Web.UC.UCBase3
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                this.AddFieldSetRed("错误", "节点(" + nd.Name + ")不能执行批量处理操作.");
                return;
            }
            ...
            if (nd.HisBatchRole == BatchRole.Group)
                this.AddCaption("批处理: " + nd.FlowName + " --> " + nd.Name + "<a href='Batch.aspx'>返回</a>&nbsp;&nbsp;<input  ID=\"btnGroup\" type=\"button\" value=\"合卷批复\" CssClass=\"Btn\" onclick=\"BatchGroup()\" />");
            else
                this.AddCaption("批处理: " + nd.FlowName + " --> " + nd.Name + "<a href='Batch.aspx'>返回</a>");
