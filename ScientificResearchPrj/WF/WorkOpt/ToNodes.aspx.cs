    ...
    public partial class ToNodes : BP.Web.WebPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            foreach (Node mynd in nds)
            {
                ...
                string hadEmpsSql = string.Format("select No, Name from Port_Emp where No in( select FK_Emp from WF_SelectAccper " +
                                               "where FK_Node={0} and WorkID={1})", mynd.NodeID, this.WorkID);
                DataTable dt = DBAccess.RunSQLReturnTable(hadEmpsSql);
                string addSpan = "";
                if (dt.Rows.Count != 0)
                {
