    ...
    public partial class WebOffice : System.Web.UI.Page
    {
        ...
        private void LoadMenu(bool isMenu)
        {
            ...
            if (isMenu && !isCompleate)
            {
                ...
                {
                    divMenu.InnerHtml =
                       "查看留痕:<select id='marks' onchange='ShowUserName()'  style='width: 100px'><option value='1'>显示留痕</option><option value='2'>隐藏留痕</option><select>";
                }
            ...
            if (!isCompleate)
            {
                if (btnLab.OfficeSaveEnable ==false)
                    ReadOnly = true;
