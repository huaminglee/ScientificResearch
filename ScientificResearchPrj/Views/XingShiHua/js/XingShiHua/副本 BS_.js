...
var addOrUpdateIndex = undefined;
    ...
    var BS_ = {
        ...
        onLoadSuccess:function(data, status){
            ...
            if (data.state == "0") {
                ...
                if (XSHOriginResult.rows.length == 1) {
                    BS_.getPropertygridDataFormJsonResult(XSHOriginResult, "XSH_");

                    //只有一个结果时，不给删除
                    var pData = $("#pg").propertygrid('getData');
                    for (var i = pData.rows.length - 1; i >= 0; i--) {
                        if (pData.rows[i].id != undefined && pData.rows[i].id == "XSH_del") {
                            $("#pg").propertygrid("deleteRow", i);
                        }
                    }
                }
                else {
                    BS_.getAndCloneXSHPropertygridDataFormJsonResult(XSHOriginResult, "XSH_");
                }
        ...
        getAndCloneXSHPropertygridDataFormJsonResult: function (result, prefix) {
            BS_.propertygridEndEdit();
        ...
        appendXSH: function () {
            ...
            $("#pg").propertygrid("insertRow", {
                ...
                row: {
                    name: "分隔位置",
                    value: ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>",
                    group: BS_.getGroupNameByPrefix("XSH_"),
                    index: targetIndex
                }
            });

            var tempData = BS_.getClonePropertygridByPrefix("XSH_", 0, 0);
            for (var j = 0; j < tempData.length; j++) {
                //更新index
        ...
        deleteRowFromPropertygrid: function () {
            ...
            for (var i = pData.rows.length - 1; i >= 0; i--) {
                if (pData.rows[i].group == BS_.getGroupNameByPrefix("XSH_")) {
            ...
            if (row.index == maxEditIndex) {
                var insertIndex = BS_.getLastInsertIndexByGroupName(BS_.getGroupNameByPrefix("XSH_"));
                for (var i = 0; i < XSHPropertygridData.length; i++) {
                    if (XSHPropertygridData[i].id == "XSH_add") {
        ...
        addSuccess: function (data, status) {
            ...
            if (data.state == "0") {
                BS_.updateXSHOriginResultFromPropertygridData("XSH_");
                lastSaveResult = true;
            } else {
        ...
        ifColumnNeedCompare: function (columnName) {
            if (columnName == "XSH_No" ||
                columnName == "XSH_Name" ||
                columnName == "XSH_ProposeTime" ||
                columnName == "XSH_FK_SLOID" ||
                columnName == "XSH_Keys" ||
                columnName == "XSH_Description" ||
                columnName == "XSH_Remarks")
                return true;
            else
                return false;
        },
        ...
        getNewRowFromPropertygrid: function (index) {
            var newRow = {};
            newRow["No"] = BS_.getValueFromPropertygridById("XSH_No", index);
            newRow["Name"] = BS_.getValueFromPropertygridById("XSH_Name", index);
            newRow["FK_Proposer"] = currentLoginUser;
            newRow["ProposeTime"] = BS_.getValueFromPropertygridById("XSH_ProposeTime", index);
            newRow["FK_SLOID"] = BS_.getValueFromPropertygridById("XSH_FK_SLOID", index);
            newRow["Keys"] = BS_.getValueFromPropertygridById("XSH_Keys", index);
            newRow["Description"] = BS_.getValueFromPropertygridById("XSH_Description", index);
            newRow["Remarks"] = BS_.getValueFromPropertygridById("XSH_Remarks", index);
