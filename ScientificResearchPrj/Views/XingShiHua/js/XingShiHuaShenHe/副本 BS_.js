...
var BS_ = {
        ...
        onLoadSuccess:function(data, status){
            ...
            if (data.state == "0") {
                XSHShenHeOriginResult = parseJSON(data._Json);

                BS_.getPropertygridDataFormJsonResult(XSHShenHeOriginResult, "XSHSH_");
                $("#pg").propertygrid("collapseGroup");
                $("#pg").propertygrid("expandGroup", 0);

                if (XSHShenHeOriginResult.rows[0].ShenHeJieGuo == SHENHEJIEGUO.RETURN) {
                    var newData = $("#pg").propertygrid('getData');
                    for (var i = 0; i < newData.rows.length; i++) {
                        if (newData.rows[i].id == "XSHSH_ShenHeJieGuo") {
        ...
        onSelectReturnToNodeCombobox: function(rec) {
            ...
            var newData = $("#pg").propertygrid('getData');
            for (var i = 0; i < newData.rows.length; i++) {
                if (newData.rows[i].id == "XSHSH_ShenHeYiJian") {
       ...
       baoCun: function () {
            ...
            if (XSHShenHeOriginResult == undefined) {
                //新增
                BS_.add();
            }
            else {
        ...
        addSuccess: function (data, status) {
            ...
            if (data.state == "0") {
                BS_.updateXSHShenHeOriginResult();
                saveResult = true;
            } else {
                saveResult = false;
            }
        ...
        update: function () {
            if (BS_.isChange() == true) {
                var newRow = BS_.getNewRowFromPropertygrid();
                newRow["oldOID"] = XSHShenHeOriginResult.rows[0].OID;
        ...
        isChange: function () {
            ...
            for (var i = 0; i < editData.rows.length; i++) {
                ...
                if (id != undefined && BS_.ifColumnNeedCompare(id) == true) {
                    ...
                    if (XSHShenHeOriginResult.rows[0][subid] != editData.rows[i].value) {
                        return true;
                    }
                }
            }
        ...
        ifColumnNeedCompare: function (columnName) {
            if (columnName == "XSHSH_ShenHeShiJian" ||
                columnName == "XSHSH_ShenHeJieGuo" ||
                columnName == "XSHSH_ShenHeYiJian")
                return true;
            else
                return false;
        },
        ...
        getNewRowFromPropertygrid: function () {
            ...
            newRow["ShenHeShiJian"] = BS_.getValueFromPropertygridById("XSHSH_ShenHeShiJian");
            newRow["ShenHeJieGuo"] = BS_.getValueFromPropertygridById("XSHSH_ShenHeJieGuo");
            newRow["ShenHeYiJian"] = BS_.getValueFromPropertygridById("XSHSH_ShenHeYiJian");
            newRow["FK_Flow"] = FK_Flow;
            newRow["FK_Node"] = FK_Node;
            newRow["WorkID"] = WorkID;
            newRow["StepType"] = StepType.XINGSHIHUA;
            return newRow;
        },

        updateXSHShenHeOriginResult: function () {
            var argsVal = {
                "FK_Flow": FK_Flow,
                "WorkID": WorkID,
                "FK_Node": FK_Node,
            ...
            $("#pg").propertygrid("loading");
            AT_.AjaxPost("/ShenHe/GetShenHeHistoryData", argsVal, BS_.updateXSHShenHeOriginResultSuccess);
        },

        updateXSHShenHeOriginResultSuccess: function (data, status) {
            $("#pg").propertygrid("loaded");
        ...
        faSong: function () {
            ...
            if (saveResult == true) {
                ...
                for (var i = 0; i < data.rows.length; i++) {
                    if (data.rows[i].id == "XSHSH_ShenHeJieGuo") {
                        //退回
                        ...
                        //正常发送
                            ...
                            window.location.href = '/XingShiHua/XingShiHuaShenHeFaQi?FK_Flow=' + FK_Flow + '&WorkID=' + WorkID;
                        }
                    }
                }
            }
        ...
        tuiHui: function () {
            ...
            AT_.AjaxPost("/XingShiHua/TuiHui", newRow, BS_.tuiHuiSuccess);
        },
        ...
        getReturnDataFromPropertygrid: function () {
            ...
            newRow["TuiHuiLiYou"] = BS_.getValueFromPropertygridById("XSHSH_ShenHeYiJian");
            newRow["FK_Flow"] =  FK_Flow;
            newRow["WorkID"] =  WorkID;
            newRow["FK_Node"] = FK_Node;
            newRow["FID"] = FID;
