...
var BS_ = {
        ...
        onLoadSuccess:function(data, status){
            ...
            if (data.state == "0") {
                SJSFShenHeOriginResult = parseJSON(data._Json);

                BS_.getPropertygridDataFormJsonResult(SJSFShenHeOriginResult, "SJSFSH_");
                $("#pg").propertygrid("collapseGroup");
                $("#pg").propertygrid("expandGroup", 0);

                if (SJSFShenHeOriginResult.rows[0].ShenHeJieGuo == SHENHEJIEGUO.RETURN) {
                    var newData = $("#pg").propertygrid('getData');
                    for (var i = 0; i < newData.rows.length; i++) {
                        if (newData.rows[i].id == "SJSFSH_ShenHeJieGuo") {
        ...
        onSelectReturnToNodeCombobox: function(rec) {
            ...
            var newData = $("#pg").propertygrid('getData');
            for (var i = 0; i < newData.rows.length; i++) {
                if (newData.rows[i].id == "SJSFSH_ShenHeYiJian") {
       ...
       baoCun: function () {
            ...
            if (SJSFShenHeOriginResult == undefined) {
                //新增
                BS_.add();
            }
            else {
        ...
        addSuccess: function (data, status) {
            ...
            if (data.state == "0") {
                BS_.updateSJSFShenHeOriginResult();
                saveResult = true;
            } else {
                saveResult = false;
            }
        ...
        update: function () {
            if (BS_.isChange() == true) {
                var newRow = BS_.getNewRowFromPropertygrid();
                newRow["oldOID"] = SJSFShenHeOriginResult.rows[0].OID;
        ...
        isChange: function () {
            ...
            for (var i = 0; i < editData.rows.length; i++) {
                ...
                if (id != undefined && BS_.ifColumnNeedCompare(id) == true) {
                    ...
                    if (SJSFShenHeOriginResult.rows[0][subid] != editData.rows[i].value) {
                        return true;
                    }
                }
            }
        ...
        ifColumnNeedCompare: function (columnName) {
            if (columnName == "SJSFSH_ShenHeShiJian" ||
                columnName == "SJSFSH_ShenHeJieGuo" ||
                columnName == "SJSFSH_ShenHeYiJian")
                return true;
            else
                return false;
        },
        ...
        getNewRowFromPropertygrid: function () {
            ...
            newRow["ShenHeShiJian"] = BS_.getValueFromPropertygridById("SJSFSH_ShenHeShiJian");
            newRow["ShenHeJieGuo"] = BS_.getValueFromPropertygridById("SJSFSH_ShenHeJieGuo");
            newRow["ShenHeYiJian"] = BS_.getValueFromPropertygridById("SJSFSH_ShenHeYiJian");
            newRow["FK_Flow"] = FK_Flow;
            newRow["FK_Node"] = FK_Node;
            newRow["WorkID"] = WorkID;
            newRow["StepType"] = StepType.SHEJISUANFA;
            return newRow;
        },

        updateSJSFShenHeOriginResult: function () {
            var argsVal = {
                "FK_Flow": FK_Flow,
                "WorkID": WorkID,
                "FK_Node": FK_Node,
            ...
            $("#pg").propertygrid("loading");
            AT_.AjaxPost("/ShenHe/GetShenHeHistoryData", argsVal, BS_.updateSJSFShenHeOriginResultSuccess);
        },

        updateSJSFShenHeOriginResultSuccess: function (data, status) {
            $("#pg").propertygrid("loaded");
        ...
        faSong: function () {
            ...
            if (saveResult == true) {
                ...
                for (var i = 0; i < data.rows.length; i++) {
                    if (data.rows[i].id == "SJSFSH_ShenHeJieGuo") {
                        //退回
                        ...
                        //正常发送
                            ...
                            window.location.href = '/SheJiSuanFa/SheJiSuanFaShenHeFaQi?FK_Flow=' + FK_Flow + '&WorkID=' + WorkID;
                        }
                    }
                }
            }
        ...
        tuiHui: function () {
            ...
            AT_.AjaxPost("/SheJiSuanFa/TuiHui", newRow, BS_.tuiHuiSuccess);
        },
        ...
        getReturnDataFromPropertygrid: function () {
            ...
            newRow["TuiHuiLiYou"] = BS_.getValueFromPropertygridById("SJSFSH_ShenHeYiJian");
            newRow["FK_Flow"] =  FK_Flow;
            newRow["WorkID"] =  WorkID;
            newRow["FK_Node"] = FK_Node;
            newRow["FID"] = FID;
        ...
        liuChengTu: function () {
            window.open('/SheJiSuanFa/ChaKanLiuChengTu?FK_Flow=' + FK_Flow);
        },
