...
namespace BP.Web.UC
{
    ...
    public class UCBase : System.Web.UI.UserControl
    {
            ...
            foreach (System.Web.UI.Control ctl in this.Controls)
            {
                ...
                if (ctlid.Contains("TB_") == true)
                {
                    ...
                    if (tb != null)
                    {
                        ...
                        {
                            if (Request.Path.EndsWith("Frm.aspx"))
                            {
                            ...
                            else if (Request.Path.EndsWith("MyFlow.aspx"))
                            {
                                foreach (string paramKey in Request.Params.AllKeys)
                                {
                                    if (paramKey.EndsWith(tb.ID))
        ...
        public void AddEasyUiLinkButton(string text, string url, string iconCls = null, bool isPlain = false)
        {
            this.Add(
                string.Format(
                    "<a class='easyui-linkbutton' href='{0}' data-options=\"plain:{1},iconCls:'{2}'\">{3}</a>&nbsp;",
                    url, isPlain.ToString().ToLower(), iconCls ?? string.Empty, text));
        }
        #endregion
