...
namespace BP.WF
{
    ...
    public class Dev2Interface
    {
            ...
            get
            {
                ...
                switch (emp.HisAuthorWay)
                {
                    case Port.AuthorWay.All:
                        if (BP.WF.Glo.IsEnableTaskPool == true)
                            ps.SQL = "SELECT count(WorkID) as Num FROM WF_EmpWorks WHERE (" + wfSql + ") AND FK_Emp=" + dbstr + "FK_Emp AND TaskSta!=0  ";
                        else
                            ps.SQL = "SELECT count(WorkID) as Num FROM WF_EmpWorks WHERE (" + wfSql + ") AND FK_Emp=" + dbstr + "FK_Emp ";
                        ps.Add("FK_Emp", BP.Web.WebUser.No);
                        break;
        ...
        public static int Todolist_Complete
        {
            get
            {
                if (Glo.IsDeleteGenerWorkFlow == false)
                {
                    ...
                    Paras ps = new Paras();
                    string dbstr = BP.Sys.SystemConfig.AppCenterDBVarStr;
                    ps.SQL = "SELECT count(WorkID) Num FROM WF_GenerWorkFlow WHERE Emps LIKE '%@" + WebUser.No + "@%' AND FID=0 AND WFState=" + (int)WFState.Complete;
        ...
        /// <param name="emailBody">邮件内容</param>
        /// <param name="msgFlag">消息标记，可以为空。</param>
        /// <param name="msgType">消息类型，可以为空。</param>
        ...
        /// <param name="parentFlowWorkID">父流程WorkID</param>
        ...
        /// <param name="flowNo">流程编号</param>
        /// <param name="workid">工作ID</param>
        /// <param name="isAutoRun">是否自动运行？ 如果自动运行，就按照流程设置的规则运行。
        /// 非自动运行，就按照用户自己定义的运转顺序计算。</param>
        /// <param name="paras">手工运行的参数格式为: @节点ID1,子流程No,接受人1,接受人n,@节点ID2,子流程No,接受人1,接受人n,</param>
        ...
        public static Int64 Node_CreateBlankWork(string flowNo, Hashtable ht, DataSet workDtls,string starter, string title)
        {
        ...
        /// <returns>为开始节点创建工作后产生的WorkID.</returns>
        public static Int64 Node_CreateBlankWork(string flowNo, Hashtable ht, DataSet workDtls, string starter, string title, Int64 parentWorkID,   string parentFlowNo, int parentNodeID, string parentEmp)
        {
            return Node_CreateBlankWork(flowNo, ht, workDtls, starter, title, 0, 0, null, 0, null, 0, null);
        }
        /// <summary>
        ...
        public static string Node_SaveWork(string fk_flow, int fk_node, Int64 workID, Hashtable htWork, DataSet dsDtls)
        {
            ...
            try
            {
                ...
                #region 保存从表
                // 执行保存事件.
     //  this.HisFlow.DoFlowEventEntity(EventListOfNode.SaveBefore, this.HisNode, this.HisWork, null);
                        ...
                        foreach (MapDtl dtl in wk.HisMapDtls)
                        {
                            ...
                            foreach (DataRow dr in dt.Rows)
                            {
                                ...
                                if (daDtl.OID == 0)
                                    ...
                                    daDtl.InsertAsOID(daDtl.OID); //插入数据.
                            }
                        }
                    }
                }
        ...
        public static string Node_Askfor(Int64 workid, AskforHelpSta askforSta, string askForEmp, string askForNote)
        {
            ...
            foreach (GenerWorkerList item in gwls)
            {
                ...
                {
                    if (item.IsCC == true)  //如果是抄送的，就可以看到的.
        ...
        public static string Node_Shift(string flowNo, int nodeID, Int64 workID, Int64 fid, string toEmp, string msg)
        {
            ...
            if (nd.TodolistModel == TodolistModel.Order || nd.TodolistModel == TodolistModel.Teamup)
            {
                ...
                info += "@<a href='" + Glo.CCFlowAppPath + "WF/MyFlowInfo.aspx?DoType=UnShift&FK_Flow=" + nd.FK_Flow + "&WorkID=" + workID + "&FK_Node=" + nodeID + "&FID=" + fid + "' ><img src='Img/UnDo.gif' border=0 />撤消工作移交</a>.";
                return info;
            ...
            //发送邮件.
            BP.WF.Dev2Interface.Port_SendMsg(emp.No, WebUser.Name + "向您移交了工作" + gwf.Title, "移交信息:" + msg, "SF" + workID + "_" + sf.FK_Node, BP.WF.SMSMsgType.Self, gwf.FK_Flow, gwf.FK_Node, gwf.WorkID, gwf.FID);
            ...
            inf1o += "@<a href='" + Glo.CCFlowAppPath + "WF/MyFlowInfo.aspx?DoType=UnShift&FK_Flow=" + nd.FK_Flow + "&WorkID=" + workID + "&FK_Node=" + nodeID + "&FID=" + fid + "' ><img src='Img/UnDo.gif' border=0 />撤消工作移交</a>.";
            return inf1o;
        }
        /// <summary>
        /// 执行工作退回(退回指定的点)
