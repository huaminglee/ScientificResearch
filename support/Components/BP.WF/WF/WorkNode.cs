...
#warning 记录日志.
        ...
        private void NodeSend_11(Node toND)
        {
            ...
            string htmlInfo = string.Format("@任务自动发送给{0}如下{1}位处理人,{2}.", this.nextStationName,
                this.HisRememberMe.NumOfObjs.ToString(), this.HisRememberMe.EmpsExt);

            string textInfo = string.Format("@任务自动发送给{0}如下{1}位处理人,{2}.", this.nextStationName,
            ...
            {
                if (WebUser.IsWap)
                    this.addMsg(SendReturnMsgFlag.AllotTask, null, "<a href=\"" + this.VirPath + "WF/WorkOpt/AllotTask.aspx?WorkID=" + this.WorkID + "&NodeID=" + toND.NodeID + "&FK_Flow=" + toND.FK_Flow + "')\"><img src='" + VirPath + "WF/Img/AllotTask.gif' border=0/>指定特定的处理人处理</a>。", SendReturnMsgType.Info);
                else
                    this.addMsg(SendReturnMsgFlag.AllotTask, null, "<a href=\"javascript:WinOpen('" + VirPath + "WF/WorkOpt/AllotTask.aspx?WorkID=" + this.WorkID + "&NodeID=" + toND.NodeID + "&FK_Flow=" + toND.FK_Flow + "')\"><img src='" + VirPath + "WF/Img/AllotTask.gif' border=0/>指定特定的处理人处理</a>。", SendReturnMsgType.Info);
            }
            ...
            if (this.HisNode.HisFormType != NodeFormType.SDKForm)
            {
                ...
                {
                    if (WebUser.IsWap)
                        this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='" + VirPath + "WF/Wap/MyFlowInfo.aspx?DoType=UnSend&WorkID=" + this.HisWork.OID + "&FK_Flow=" + toND.FK_Flow + "'><img src='" + VirPath + "WF/Img/UnDo.gif' border=0/>撤销本次发送</a>， <a href='" + VirPath + "WF/Wap/MyFlow.aspx?FK_Flow=" + toND.FK_Flow + "&FK_Node=" + toND.FK_Flow + "01'><img src='" + VirPath + "WF/Img/New.gif' border=0/>新建流程</a>。", SendReturnMsgType.Info);
                    else
                        this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='" + this.VirPath + this.AppType + "/MyFlowInfo.aspx?DoType=UnSend&WorkID=" + this.HisWork.OID + "&FK_Flow=" + toND.FK_Flow + "'><img src='" + VirPath + "WF/Img/UnDo.gif' border=0/>撤销本次发送</a>， <a href='" + VirPath + "WF/MyFlow.aspx?FK_Flow=" + toND.FK_Flow + "&FK_Node=" + toND.FK_Flow + "01'><img src='" + VirPath + "WF/Img/New.gif' border=0/>新建流程</a>。", SendReturnMsgType.Info);
                }
                else
                    this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='" + this.VirPath + this.AppType + "/MyFlowInfo.aspx?DoType=UnSend&WorkID=" + this.HisWork.OID + "&FK_Flow=" + toND.FK_Flow + "'><img src='" + VirPath + "WF/Img/UnDo.gif' border=0/>撤销本次发送</a>。", SendReturnMsgType.Info);
            }
        ...
        private void NodeSend_24_SameSheet(Node toNode)
        {
            ...
            gwls.Delete(GenerWorkerListAttr.FK_Node, this.HisNode.NodeID, GenerWorkerListAttr.FID, this.WorkID); //首先清除.
            ...
            #region 处理消息提示
            string info = "@分流节点:{0}已经发起,@任务自动发送给如下(" + this.HisRememberMe.NumOfObjs + ")个处理人{1}.";
            ...
            if (this.HisNode.IsStartNode)
            {
                if (gwls.Count >= 2 && this.HisNode.IsTask)
                    this.addMsg("AllotTask", "@<img src='" + VirPath + "WF/Img/AllotTask.gif' border=0 /><a href=\"" + VirPath + "WF/WorkOpt/AllotTask.aspx?WorkID=" + this.WorkID + "&FID=" + this.WorkID + "&NodeID=" + toNode.NodeID + "\" >修改接受对象</a>.");
            }

            if (this.HisNode.IsStartNode)
            {
                if (WebUser.IsWap)
                    this.addMsg("UnDoNew", "@<a href='" + VirPath + "WF/Wap/MyFlowInfo.aspx?DoType=UnSend&WorkID=" + this.WorkID + "&FK_Flow=" + toNode.FK_Flow + "'><img src='" + VirPath + "WF/Img/UnDo.gif' border=0/>撤销本次发送</a>， <a href='" + VirPath + "WF/Wap/MyFlow.aspx?FK_Flow=" + toNode.FK_Flow + "&FK_Node=" + toNode.FK_Flow + "01' ><img src='" + VirPath + "WF/Img/New.gif' border=0/>新建流程</a>。");
                else
                    this.addMsg("UnDoNew", "@<a href='" + this.VirPath + this.AppType + "/MyFlowInfo.aspx?DoType=UnSend&WorkID=" + this.WorkID + "&FK_Flow=" + toNode.FK_Flow + "'><img src='" + VirPath + "WF/Img/UnDo.gif' border=0/>撤销本次发送</a>， <a href='" + this.VirPath + this.AppType + "/MyFlow.aspx?FK_Flow=" + toNode.FK_Flow + "&FK_Node=" + toNode.FK_Flow + "01' ><img src='" + VirPath + "WF/Img/New.gif' border=0/>新建流程</a>。");
            }
            else
            {
                this.addMsg("UnDo", "@<a href='" + this.VirPath + this.AppType + "/MyFlowInfo.aspx?DoType=UnSend&WorkID=" + this.WorkID + "&FK_Flow=" + toNode.FK_Flow + "'><img src='" + VirPath + "WF/Img/UnDo.gif' border=0/>撤销本次发送</a>。");
            }
...
#warning 如果这里设置了自动跳转，现在去掉了. 2014-11-07.
        ...
        public void SendMsgToThem(GenerWorkerLists gwls)
        {
            ...
            //        msgTemp = "新工作" + this.rptGe.Title + "发送人:" + WebUser.No + "," + WebUser.Name + ",流程:" + this.HisFlow.Name;
            //    else
