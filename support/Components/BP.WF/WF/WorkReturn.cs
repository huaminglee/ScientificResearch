    ...
    public class WorkReturn
    {
        ...
        private void DeleteSpanNodesGenerWorkerListData()
        {
            ...
            if (dt.Rows.Count >= 1)
            {
                ...
                ps.Clear();
                ps.SQL = "SELECT ActionType,NDFrom FROM ND" + int.Parse(this.HisNode.FK_Flow) + "Track WHERE   RDT >=" + dbStr + "RDT ORDER BY RDT ";
                ps.Add("RDT", rdt);
                ps.Add("WorkID", this.WorkID);
        ...
        public string DoOrderReturn()
        {
            ...
            {
                // 记录退回日志.
                ReorderLog(this.HisNode, this.ReurnToNode, rw);
            }
            catch (Exception ex)
            {
...
#warning 没有考虑两种流程数据存储模式。
            ...
            if (this.ReurnToNode.IsGuestNode)
            {
                ...
                return "工作已经被您退回到(" + this.ReurnToNode.Name + "),退回给(" + gwf.GuestNo + "," + gwf.GuestName + ").\n\r" + text;
            }
            else
            {
                return "工作已经被您退回到(" + this.ReurnToNode.Name + "),退回给(" + returnToEmp.No + "," + returnToEmp.Name + ").\n\r" + text;
            }
        }
        /// <summary>
        /// 执行退回.
        ...
        private string ExeReturn2_4()
        {
            ...
            gwls.Retrieve(GenerWorkerListAttr.WorkID, this.WorkID, GenerWorkerListAttr.FK_Node, this.ReurnToNode.NodeID);
            ...
            return "成功的把信息退回到：" + this.ReurnToNode.Name + " , 退回给:(" + this.ReturnToEmp + ")";
        }
        /// <summary>
        /// 子线程退回给分流点
        /// </summary>
        ...
        private string ExeReturn5_2()
        {
            ...
            gwf.FK_Node = this.ReurnToNode.NodeID;
            gwf.NodeName = this.ReurnToNode.Name;
            gwf.Starter = toEmp;
        ...
        private string ExeReturn1_1()
        {
            ...
            try
            {
                // 记录退回日志. this.HisNode, this.ReurnToNode
